...
- **batch 维度**：TOPIQ-NR/IAA/NR-Face（2026-08-29 pyiqa 重导出）为**动态 batch**（整批一次推理，Rust 侧分批 8）；OCEC/scene/MobileNet 等仍 **fix batch=1**，必须逐张推理（批量输入静默失败）。nr_face 批量化后旧模型回退逻辑保留（`face_quality_scores` 批量失败自动逐张）。SCRFD 人脸检测：`det_10g_batched.onnx`（`scripts/make_det_batched.py` 图手术生成，权重不变）存在即启用批量检测，**默认不随包发布**（生产链路实测无净收益且拖慢 HyperIQA，见 GPU_PERF_PLAN.md）；缺失时走 2 副本会话池并行逐张。
