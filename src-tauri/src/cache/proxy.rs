...
//!
//! **大小对比基准（重要约定）**：代理图只参与**图像内容分析**（感知哈希指纹、
//! AI 推理、对焦、闭眼等）。任何**文件大小/体积比较**（综合分启发式、推荐
//! tie-break、推荐理由等）必须用**源文件大小**（`ImageInfo.size`，walker 阶段
//! 由 `fs::metadata().len()` 写入）——代理体积在整个评分/推荐链路中不得被读取。
//! 代理只返回像素（`ai_proxy -> RgbImage`），不携带任何元数据，即为此约定的
//! 结构性保证。
//!
